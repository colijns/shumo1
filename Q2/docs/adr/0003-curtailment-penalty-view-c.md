...
> **状态：主结论已被 ADR 0004 取代（2026-07-25）；λ 法降级为补充敏感性分析**
> 本 ADR 的 λ 法实现、λ=0 锚定 0/0、λ>0 推动配置翻正的机理分析**仍然有效**，
> 代码 `Q2/q2_storage_penalty.py` 与输出 `Q2/output_penalty*/` 完全不动。
> 被取代的仅是"λ=0.3 为推荐方案"的主结论定位--ADR 0004 改以 ε-约束 Pareto
> 前沿的 97% 折中点（175 kW/890 kWh）为主结论，λ 法重新定位为"弃电等效价值"
> 的补充敏感性分析，独立成文 `docs/问题2（2）lambda敏感性.md`。详见
