...
def run_q1(output_dir=None):
    ...
    for park in parks:
        ...
        # 最终工程方案调度图
        final = final_results.get(park, {}).get('engineering')
        if final and final['success'] and final['E_ess'] > 0:
            plot_storage_dispatch(
                final, park,
                f'工程最优{final["P_ess"]:.0f}kW/{final["E_ess"]:.0f}kWh',
                final['P_ess'], final['E_ess'],
    ...
    # 使用统一工程粒度绘制最终方案附近的成本曲面
    for park in ['B', 'C']:
        gs = local_grids.get(park)
        if not gs:
            continue
        plot_cost_heatmap(
            gs['grid'], park,
